...
config.config.baseln = 5; % length of pause at begining of each run, sec
...
config.prcnt_go_p_alone = 0.95; % percentage of trials that are go trials
config.prcnt_go_p_opto = 0.5; % percentage of trials that are go trials
...
config.sig_amps = [3]; % amplitudes of stimuli, Volts
% =====>** need to fix this: prcnt_amps = [1]; % proportion of different amplitudes to present - needs to add to 1
...
config.tp.pairDelay =  500; % if doing pairing, offset between stim and reward, double, in seconds, but will be rounded to nearest integer of val * teensy_fs, e.g., 0.2112 * 2000 = 442 points or 0.221 sec   
config.tp.earlyLen =   200; % length of time to braodcast an outcome of an early response, double, in seconds, but will be rounded to nearest integer of val * teensy_fs, e.g., 0.2112 * 2000 = 442 points or 0.221 sec   
...
for i = 1:numel(config.opto_times)
    for j = 1:numel(config.sig_amps)
        ...
        trls = [trls; ((i*10) + j) * ones(ceil((n_opto_trials/n_opto_ttypes)*config.prcnt_go_p_opto),1)]; 
    end
end
...
while f.UserData.state ~= 3
    ...
    elseif f.UserData.state == 1 % detection or pairing run
        ...
        axc.Children.XData = config.sig_amps;
